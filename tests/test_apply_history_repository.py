...
class ApplyHistoryRepositoryTests(unittest.TestCase):
    def test_fetch_rollback_items_returns_successful_apply_mutations_in_reverse_order(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with connect_sqlite(db_path) as connection:
                ...
                repository = ApplyHistoryRepository(connection)

                items = repository.fetch_rollback_items(execution_run_id=execution_run_id)

            self.assertEqual(["op-3", "op-1"], [item.operation_log_id for item in items])
            self.assertEqual([3, 1], [item.sequence_no for item in items])
            self.assertEqual(
                ["copy_delete", "move"],
                [item.performed_action for item in items],
            )

    def test_fetch_rollback_items_excludes_dry_run_skip_and_failed_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            db_path = Path(tmp_dir) / "state.db"
            ...
            self.assertTrue(all(item.result == "success" for item in items))
